...
async function cli(provider, model, prompt, timeoutMs = 180000, opts = {}) {
  ...
  if (provider === "openai") { const P = require("./providers"); model = P.resolveOpenAIModel(model).model; }   // an id the installed Codex accepts
  const eff = opts.effort ? require("./providers").openaiEffort(opts.effort, model) : null;

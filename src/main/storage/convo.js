...
function derive(messages) {
  ...
  for (let i = n - 1; i >= stop; i--) {
    const m = messages[i];
    if (!m || m.parentToolUseId) continue;   // the primary conversation only: a sub-agent's own tool calls / text are not the conversation's outcomes
...
/* The SESSION MAP — structured facts about one conversation for a handoff into a fresh session
 * ("Synthesize → new session"): what the user pursued, what came out of it, the files and tools
 * involved, how big it was. Derived deterministically (no model call), never a transcript. */
function sessionMap(session) {
  const m = merge(session);
  const msgs = (session && session.messages) || [];
  let userTurns = 0, assistantTurns = 0, toolCalls = 0, firstTs = "", lastTs = "";
  for (const x of msgs) {
    if (!x || x.parentToolUseId) continue;   // sub-agent internal entries are not counted as the conversation's turns / tool calls
    if (x.role === "user") userTurns++; else if (x.role === "assistant") assistantTurns++; else if (x.role === "tool") toolCalls++;
    if (x.ts) { if (!firstTs || x.ts < firstTs) firstTs = x.ts; if (!lastTs || x.ts > lastTs) lastTs = x.ts; }
  }
  const files = m.files.slice().sort((a, b) => (b.count - a.count) || (b.added - a.added)).slice(0, 40);
  const tools = Object.keys(m.tools).sort((a, b) => m.tools[b] - m.tools[a]).slice(0, 20).map((k) => ({ name: k, count: m.tools[k] }));
  // merge() lists newest first; a handoff reads in the order things happened (oldest → newest)
  return { total: m.liveCount + m.prunedCount, live: m.liveCount, pruned: m.prunedCount, userTurns, assistantTurns, toolCalls, firstTs, lastTs, goals: m.goals.slice(0, 12).reverse(), outcomes: m.outcomes.slice(0, 10).reverse(), files, tools };
}
// The map as a labelled block of facts (no instructions), bounded to `budget` characters. Empty
// when there is nothing to say (an empty or one-line conversation).
function sessionMapText(map, budget = 4800) {
  if (!map || (!map.total && !map.goals.length && !map.files.length)) return "";
  const when = (ts) => String(ts || "").slice(0, 16).replace("T", " ");
  const span = map.firstTs && map.lastTs && map.firstTs !== map.lastTs ? `, ${when(map.firstTs)} → ${when(map.lastTs)}` : "";
  const parts = [`[Session map — facts distilled from the source conversation, not a transcript: ${map.total} entr${map.total === 1 ? "y" : "ies"} (${map.userTurns} user turn${map.userTurns === 1 ? "" : "s"}, ${map.assistantTurns} assistant repl${map.assistantTurns === 1 ? "y" : "ies"}, ${map.toolCalls} tool call${map.toolCalls === 1 ? "" : "s"})${span}]`];
  const list = (label, items, cap) => { if (items && items.length) parts.push(`${label}:\n` + items.slice(0, cap).map((t) => `- ${t}`).join("\n")); };
  list("Goals the user pursued", map.goals, 12);
  list("Outcomes reported", map.outcomes, 10);
  if (map.files.length) parts.push("Files worked on (edits, +added/−removed lines):\n" + map.files.slice(0, 16).map((f) => `- ${f.path} (${f.count}×, +${f.added}/−${f.removed})`).join("\n"));
  if (map.tools.length) parts.push("Tools used: " + map.tools.slice(0, 10).map((t) => `${t.name} ${t.count}×`).join(", "));
  let out = parts.join("\n\n");
  if (out.length > budget) out = out.slice(0, budget - 1) + "…";
  return out;
}
...
module.exports = { setDir, foldPruned, digestFor, peek, remove, sessionMap, sessionMapText };

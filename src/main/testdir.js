...
function classifyByImports(source, file) {
  try { const a = require("./ast").classify(source, file); if (a) return a; } catch { /* fall back to regex below */ }
...
function assertionCount(t) {
  ...
  try { const a = require("./ast").assertions(src); if (a) return a.count; } catch { /* fall back */ }
  return (src.match(/\b(?:expect|assert)\s*\(/g) || []).length + (src.match(/toMatchSnapshot/g) || []).length;
}
function hasSkipOnly(t) {
  const src = String((t && t.source) || "");
  try { const a = require("./ast").assertions(src); if (a) return a.hasSkipOnly; } catch { /* fall back */ }

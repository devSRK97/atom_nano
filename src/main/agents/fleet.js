...
class Fleet {
  ...
  _runner() { return this.runner || ((sid, opts) => require("../session/index").run(sid, opts)); }
  maxConcurrent() {
  ...
  async cancel(id) {
    ...
    if (!t) return false;
    if (t.status === "running") { try { await require("../session/index").interrupt(t.sessionId, "stop"); } catch { /* ignore */ } }

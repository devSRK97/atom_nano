 ...
 *   model.ctx / ctx1m        → the model's context capacity, used automatically
 *
...
const CATALOG = {
  anthropic: {
    ...
    models: [
      ...
      { id: "claude-sonnet-5", name: "Sonnet 5", desc: "Balanced speed and intelligence", ctx: 1000000, ctx1m: true },
  ...
  openai: {
    ...
    models: [
      { id: "gpt-6-astra", name: "GPT-6-Astra", desc: "Our most capable model for complex, demanding work.", ctx: 1050000, efforts: ["low", "medium", "high", "xhigh", "max", "ultra"], defaultEffort: "low" },
      { id: "gpt-5.6-sol", name: "GPT-5.6-Sol", desc: "Latest frontier agentic coding model.", ctx: 1050000, efforts: ["low", "medium", "high", "xhigh", "max", "ultra"], defaultEffort: "low" },
      { id: "gpt-5.6-terra", name: "GPT-5.6-Terra", desc: "Balanced agentic coding model for everyday work.", ctx: 1050000, efforts: ["low", "medium", "high", "xhigh", "max", "ultra"], defaultEffort: "medium" },
      { id: "gpt-5.6-luna", name: "GPT-5.6-Luna", desc: "Fast and affordable agentic coding model.", ctx: 1050000, efforts: ["low", "medium", "high", "xhigh", "max"], defaultEffort: "medium" },
...
function codexCatalog() {
  try { if (!_codexMod) _codexMod = require("./codex-models"); return _codexMod.load(); } catch { return null; }
}
...
function openaiLive(p) {
  ...
  const models = (fromCatalog ? live : p.models).map((m) => ({ id: m.id, name: m.name, desc: shortDesc(m.desc, m.upgrade ? 24 : 40) + retireNote(m), ctx: m.ctx || null, ctx1m: +m.ctx >= 1000000, efforts: m.efforts || [], defaultEffort: m.defaultEffort || "", upgrade: m.upgrade || null }));
...
function get(provider) {
  ...
  if (provider === "openai") return openaiLive(p);
  return { ...p, models: p.models.map(withContextCapacity) };
}
// Numeric capability metadata travels through discovery and into the backend's
// budget calculation. A 1M-capable model always gets that capacity; a legacy
// checkbox is not a reason to replace it with the provider's 200K fallback.
function withContextCapacity(model) {
  const numeric = +model.ctx;
  const ctx = Math.max(Number.isFinite(numeric) && numeric > 0 ? numeric : 0, model.ctx1m ? 1000000 : 0) || null;
  return { ...model, ctx, ctx1m: ctx >= 1000000 };
}
...
function context1M(provider, modelId) {
  ...
  if (m) return !!m.ctx1m || +m.ctx >= 1000000;
...
async function fetchModels(provider, token) {
  ...
  try {
    ...
    if (provider === "anthropic") {
      ...
      // the context budget reflects the model's actual capability, not an id heuristic.
      return (j.data || []).map((m) => ({
        id: m.id,
...
async function discover(provider, { claudeRef, customModels, customEndpoints, keys, force } = {}) {
  ...
  if (provider === "openai") { try { if (!_codexMod) _codexMod = require("./codex-models"); await _codexMod.refresh({ force: !!force, timeoutMs: force ? 30000 : 10000 }); } catch { /* fall back */ } }
  const p = get(provider);
  ...
  if (provider === "custom") {
    ...
    if (eps.length) models = eps.map((e) => ({ id: e.id, name: e.name || e.id, desc: "Custom API", ctx: e.ctx || e.contextWindow || null, ctx1m: !!e.ctx1m }));
    else if (Array.isArray(customModels)) models = customModels.filter((m) => m && m.id).map((m) => ({ id: m.id, name: m.name || m.id, desc: "Custom model", ctx: m.ctx || m.contextWindow || null, ctx1m: !!m.ctx1m }));
  }
  ...
  if (provider !== "custom") {
    try {
      ...
      if (dyn.length) {
        const have = new Set(models.map((m) => m.id));
        const byId = new Map(dyn.filter((m) => m && m.id).map((m) => [m.id, m]));
        // Refresh KNOWN models too. The previous merge discarded their API context
        // metadata and also dropped numeric windows from newly discovered models.
        models = models.map((m) => {
          const live = byId.get(m.id);
          if (!live) return m;
          const numeric = Number.isFinite(+live.ctx) && +live.ctx > 0;
          if (numeric) return { ...m, ctx: +live.ctx, ctx1m: +live.ctx >= 1000000 };
          if (typeof live.ctx1m === "boolean") return { ...m, ctx: live.ctx1m ? Math.max(+m.ctx || 0, 1000000) : null, ctx1m: live.ctx1m };
          return m;
        });
        let fresh = dyn.filter((m) => m && m.id && !have.has(m.id)).map((m) => ({ id: m.id, name: m.name || m.id, desc: "Discovered", ctx: m.ctx || null, ctx1m: typeof m.ctx1m === "boolean" ? m.ctx1m : +m.ctx >= 1000000 }));
  ...
  models = models.map(withContextCapacity);
  // The same discovered capacities must reach primary runs, summaries and role
  // jobs, not just the dropdown. Codex reads its account-specific catalog live.
  if (provider !== "openai" && CATALOG[provider]) CATALOG[provider].models = models;
...
// sign-in, or update. Fire-and-forget; listeners (ipc/providers.js) broadcast changes.
function refreshCodexModels(opts) { try { if (!_codexMod) _codexMod = require("./codex-models"); return _codexMod.refresh({ force: true, ...(opts || {}) }); } catch { return Promise.resolve(null); } }
function onCodexModelsChange(cb) { try { if (!_codexMod) _codexMod = require("./codex-models"); return _codexMod.onChange(cb); } catch { return () => {}; } }

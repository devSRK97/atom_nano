...
function binary() {
  try { const cm = require("./codex-models"); const list = cm.candidates(); return list[0] || ""; } catch { return ""; }
}
...
async function run({ apiKey, model, effort, cwd, promptText, images, files, resumeId, mode, summary, config, webSearch, reasoningSummary, signal, on, decide, beforeTurn } = {}) {
  ...
  let guard;   // TurnInterruptGuard — created right after `st` (it needs the turn state)
  ...
  const end = (res) => {
    ...
    st.finished = true; clearTimeout(st.retryTimer); guard.dispose(); s.threads.delete(threadId); finish(res);
  ...
  guard = new TurnInterruptGuard({
    onTurnId: (id) => { st.turnId = id; call(on.onTurnId, id); },
    interrupt: (turnId) => interrupt(threadId, turnId),
    onTimeout: () => end({ ok: false, aborted: true, threadId, turnId: st.turnId, usage: st.usage, text: st.text.join("\n\n") }),
  ...
  const ctx = {
    ...
    onNotification: (method, p) => {
      ...
      switch (method) {
        ...
        case "turn/started": if (p.turn && p.turn.id) guard.setTurnId(p.turn.id); break;
        case "item/started": {
  ...
  // A Stop that lands while turn/start is still in flight has no turn id to interrupt yet: the
  // guard remembers it and sends turn/interrupt the moment the id arrives (from the notification
  // or the turn/start response — whichever is first), so the server never keeps generating a
  // turn the app already gave up on. The fallback ends our side if the server does not confirm.
  const doInterrupt = () => guard.request();
  ...
  try {
    ...
    if (r && r.turn && r.turn.id) guard.setTurnId(r.turn.id);
  } catch (e) {
...
/* Interrupting a turn needs its id, and a Stop can arrive before turn/start has answered. The
 * guard turns "interrupt as soon as possible" into a state machine: request() interrupts now
 * when the id is known, otherwise arms a pending interrupt that setTurnId() fires the moment the
 * id lands (notification or response, whichever is first). An interrupt the server never
 * confirms is ended on our side after `timeoutMs`. Exported for the unit suite. */
class TurnInterruptGuard {
  constructor({ onTurnId, interrupt, onTimeout, isFinished, timeoutMs = 4000 } = {}) {
    this.turnId = null; this.pending = false; this.sent = false; this.timer = null;
    this.onTurnId = onTurnId || (() => {}); this.interrupt = interrupt || (async () => true); this.onTimeout = onTimeout || (() => {}); this.isFinished = isFinished || (() => false); this.timeoutMs = timeoutMs;
  }
  setTurnId(id) {
    if (!id || id === this.turnId) return false;
    this.turnId = id; this.onTurnId(id);
    if (this.pending) { this.pending = false; this.request(); }
    return true;
  }
  request() {
    if (this.isFinished() || this.sent) return this.sent ? "sent" : "finished";
    if (!this.turnId) { this.pending = true; return "deferred"; }
    this.sent = true;
    Promise.resolve().then(() => this.interrupt(this.turnId)).catch(() => {});
    this.timer = setTimeout(() => { this.timer = null; if (!this.isFinished()) this.onTimeout(); }, this.timeoutMs);
    if (this.timer.unref) this.timer.unref();
    return "sent";
  }
  dispose() { if (this.timer) { clearTimeout(this.timer); this.timer = null; } this.pending = false; }
}
...
module.exports = {
  ...
  __internals: { onData, isThreadLost, isRequirementsError, StringDecoder, onServerRequest, servers, threadOwner, TurnInterruptGuard },
};

...
const CACHE_VER = 2;   // bump when normalize() gains fields, so stale disk caches are re-parsed
...
const sig = (models) => (models || []).map((m) => `${m.id}:${m.visibility}:${(m.efforts || []).join(",")}`).join("|");

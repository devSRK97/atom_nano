 ...
 *
 *   session.bindings[provider] = { id, syncedIndex, account }
...
// Which canonical messages count as conversation history for a model.
...
const headShort = (n) => `Conversation record — ${n} earlier entr${n === 1 ? "y" : "ies"} of this same conversation, in order. Conversation text is verbatim; long tool inputs and outputs were shortened to fit the model's context window (re-read a file if you need its current content). ` + TAIL_NOTE;
const headSummary = (plan) => `Conversation record — this same conversation so far. The full record (${fmtN(plan.count)} entries, ${fmtN(plan.fullChars)} characters) is larger than the model's context window, so the ${fmtN(plan.headCount)} oldest entr${plan.headCount === 1 ? "y is" : "ies are"} carried as a summary and the ${fmtN(plan.count - plan.headCount)} most recent entr${plan.count - plan.headCount === 1 ? "y" : "ies"} verbatim (long tool inputs/outputs shortened). ` + TAIL_NOTE;
...
function planTransfer(session, from, to, { budgetChars = Infinity, forceSummary = false, tailShare = 0.5 } = {}) {
  const all = range(session, from + 1, to);
  const msgs = [];
  all.forEach((m, i) => { if (isHistoryMessage(m)) msgs.push({ m, g: from + 1 + i }); });
  if (!msgs.length) return { mode: "exact", msgs, texts: [], text: "", count: 0, chars: 0, fullChars: 0, headCount: 0 };
  const exact = msgs.map((x) => entryText(x.m));
  const exactText = headExact(msgs.length) + "\n\n" + exact.join(SEP);
  if (!forceSummary && exactText.length <= budgetChars) return { mode: "exact", msgs, texts: exact, text: exactText, count: msgs.length, chars: exactText.length, fullChars: exactText.length, headCount: 0 };
  const short = msgs.map((x) => entryTextShort(x.m));
  const shortText = headShort(msgs.length) + "\n\n" + short.join(SEP);
  if (!forceSummary && shortText.length <= budgetChars) return { mode: "shortened", msgs, texts: short, text: shortText, count: msgs.length, chars: shortText.length, fullChars: exactText.length, headCount: 0 };
  // Newest entries verbatim within `tailShare` of the budget; everything before them is summarised.
  const tailBudget = Math.max(0, Math.floor((Number.isFinite(budgetChars) ? budgetChars : shortText.length) * tailShare));
  ...
  if (tailStart === 0) {   // the whole record fits the tail share (newest entry possibly excerpted) — nothing left to summarise
    const text = headShort(msgs.length) + "\n\n" + short.join(SEP);
    return { mode: "shortened", msgs, texts: short, text, count: msgs.length, chars: text.length, fullChars: exactText.length, headCount: 0 };
  }
  return { mode: "summary", msgs, texts: short, count: msgs.length, fullChars: exactText.length, headCount: tailStart, head: { from, to: msgs[tailStart - 1].g }, tail: { from: msgs[tailStart - 1].g, to }, budgetChars, tailShare };
}
...
function transferItems(plan, summary) {
  ...
  if (plan.mode === "summary") {
    items.push(userItem({ text: `[Conversation summary — the ${fmtN(plan.headCount)} oldest entries of this same conversation, condensed because the full record (${fmtN(plan.fullChars)} characters) is larger than the model's context window. Tool entries below are completed results, not requests to run again.]\n\n${String(summary || "").trim()}` }));
    start = plan.headCount;
  }
  for (let i = start; i < plan.msgs.length; i++) { const m = plan.msgs[i].m; items.push(m.role === "user" ? userItem(m) : assistantItem(plan.texts[i])); }
  return items;
}
...
module.exports = { lastGlobalIndex, indexOfMessage, bindingFor, setBinding, dropBinding, transcriptBlock, codexItems, pendingSync, isHistoryMessage, entryText, entryTextShort, shortenText, excerptText, segmentText, planTransfer, transferText, transferItems, cachedSummary, rememberSummary, extendHead, approxTokens, CHARS_PER_TOKEN, MAX_SUMMARIES };
